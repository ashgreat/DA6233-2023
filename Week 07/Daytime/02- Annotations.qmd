...
```{r}
 economics %>% 
  ...
  annotation_raster(readPNG("UTSA_Logo_Website_UTSA_Logo_Stacked_Center.png"),
                    xmin = -Inf, xmax = Inf, ymin = 0.015, ymax = 0.04, interpolate = T) +
...
```{r}
p2 +
  labs(title = "**Rise** and **_fall_** in the economy are cyclical") +
...
') +
  theme(plot.title = element_textbox_simple(lineheight = 1.2, fill = "lightsteelblue1",
                                            padding = margin(7, 5.5, 5.5, 7, unit = "pt"),
                                            margin = margin(0, 0, 0, 10)))
```

```{r}

text_dt = data.frame(date = as.Date("1992-01-01"), 
                     unemploy_per = 0.051, 
                     mylabel = '<span style = "color: red">**Reagen Recession<sup>©</sup>** </span>')
