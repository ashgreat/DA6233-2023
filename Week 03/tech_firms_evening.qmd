...
Use `mutate()` to change the formatting of variables

Learn more about `percent()` function from `scales` package, which is a part of `tidyverse`. This function is now superseded by `label_percent()` function, which is more complex to use and you don't need to switch to it just yet.
